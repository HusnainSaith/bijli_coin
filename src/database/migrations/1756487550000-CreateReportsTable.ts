...
export class CreateReportsTable1756487550000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.createTable(new Table({
      ...
      foreignKeys: [
        { columnNames: ["reporter_id"], referencedTableName: "users", referencedColumnNames: ["id"], onDelete: "CASCADE" }
      ]
    }), true);

    await queryRunner.query('CREATE INDEX "IDX_reports_reporter_id" ON "reports" ("reporter_id")');
    await queryRunner.query('CREATE INDEX "IDX_reports_reportable" ON "reports" ("reportable_type", "reportable_id")');
  }
