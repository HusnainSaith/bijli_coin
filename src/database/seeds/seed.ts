...
async function seed() {
  try {
    ...
    const existingSuperAdminPermissionIds =
      existingSuperAdminRolePermissions.map((rp) => rp.permission_id);
    ...
    if (newSuperAdminRolePermissions.length > 0) {
      ...
      console.log(
        `Assigned ${newSuperAdminRolePermissions.length} permissions to Super Admin`,
      );
    }
    ...
    const userPermissionSlugs = [
      ...
      'comment_replies_update',
      'comment_replies_delete',
      // Bookmarks
      'bookmarks_create',
      'bookmarks_list',
      'bookmarks_delete',
      ...
      'tags_list',
      'categories_list',
      'categories_followers',
      'category_followers_create',
      'category_followers_list',
      'category_followers_delete',
      // Media
      'media_create',
      'media_list',
      'media_show',
      ...
      'answers_delete',
      // Followers
      'author_followers_create',
      'author_followers_list',
      'author_followers_delete',
    ...
    const existingUserPermissionIds = existingUserRolePermissions.map(
      (rp) => rp.permission_id,
    );
    ...
    if (newUserRolePermissions.length > 0) {
      ...
      console.log(
        `Assigned ${newUserRolePermissions.length} permissions to the 'user' role.`,
      );
    } else {
