...
export class UsersService {
    ...
    async create(createUserDto: CreateUserDto): Promise<User> {
        ...
        if (createUserDto.role) {
            const role = await this.roleRepository.findOne({ where: { slug: createUserDto.role } });
    ...
    async uploadProfilePicture(userId: string, filename: string): Promise<UserProfile> {
        const profile = await this.userProfileRepository.findOne({ where: { user_id: userId } });
        ...
        if (profile.profile_picture) {
            const oldFilePath = path.join(process.cwd(), 'uploads', 'profile-pictures', profile.profile_picture);
            if (fs.existsSync(oldFilePath)) {
                fs.unlinkSync(oldFilePath);
            }
        }

        await this.userProfileRepository.update(profile.id, { profile_picture: filename });
        return this.getProfile(userId);
    }
    ...
    async deleteProfile(userId: string): Promise<void> {
        ...
        if (profile.profile_picture) {
            const filePath = path.join(process.cwd(), 'uploads', 'profile-pictures', profile.profile_picture);
